...
def setup_venv():
    ...
    if not main_script.exists() or "env_manager" in main_script.name.lower():
        possible_paths = [
            Path.cwd() / "scripts" / "punctuation_restore.py",
            Path.cwd() / "punctuation_restore.py",
            Path(__file__).parent.parent / "scripts" / "punctuation_restore.py",   # 项目根目录/scripts/punctuation_restore.py
        ]
        for p in possible_paths:
            if p.exists():
                main_script = p
